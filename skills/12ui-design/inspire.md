...
`balanced` is the default mode. Use `--mode direct|balanced|adventurer` to trade literal query adherence against exploration, and `--reference-image <image.png>` to rank against an existing interface. Preserve manifest order: it is ranked and diversified.
